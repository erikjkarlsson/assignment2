...
static size_t primes[] = {17,   31,   67,   127,  257,  509,
                          1021, 2053, 4099, 8191, 16381};

static entry_t *entry_create(elem_t new_key, elem_t new_value,
                             entry_t *new_next) {
  entry_t *new_entry;
...
ioopm_hash_table_t *ioopm_hash_table_create_dynamic(
    ioopm_hash_function hf, ioopm_eq_function comp_key,
    ioopm_eq_function comp_value, double load_factor, size_t capacity) {
  ...
  ht->growth_stage =
      0; /// Amount of times the hash table has been resized, used as an index
...
ioopm_hash_table_t *ioopm_hash_table_create(ioopm_hash_function hf,
                                            ioopm_eq_function comp_key,
                                            ioopm_eq_function comp_value) {
  return ioopm_hash_table_create_dynamic(hf, comp_key, comp_value, Load_factor,
                                         primes[0]);
}

/// Returns true if it's time to grow the hash table
static bool check_capacity(ioopm_hash_table_t *ht, float load_factor,
...
static void ioopm_hash_table_resize(ioopm_hash_table_t *ht) {
  ht->size = 0;
  entry_t **previous_buckets =
      ht->buckets; /// Save the old buckets for later insertion
  ...
  entry_t *next = NULL;
  ht->buckets =
      create_buckets(ht, ht->capacity); /// Overwrites all current buckets with
                                        /// null-buckets, and expands hash table
  ...
  for (size_t i = 0; i < previous_capacity; i++) {
    ...
    while (entry) {
      next = entry->next;
      ioopm_hash_table_insert(ht, entry->key,
                              entry->value); /// Reinsert previous entries
      free(entry);
      entry = next;
    }
...
entry_t *find_previous_entry_for_key(ioopm_eq_function comp_key, entry_t *entry,
                                     elem_t key) {
...
void ioopm_hash_table_insert(ioopm_hash_table_t *ht, elem_t key, elem_t value) {
  ...
  entry_t *entry =
      find_previous_entry_for_key(ht->comp_key, ht->buckets[bucket], key);
...
size_t ioopm_hash_table_size(ioopm_hash_table_t *ht) { return ht->size; }
...
bool ioopm_hash_table_is_empty(ioopm_hash_table_t *ht) { return ht->size == 0; }
...
size_t ioopm_return_capacity(ioopm_hash_table_t *ht) { return ht->capacity; }
