...
#define DBG(DO)  if (opt->debug_p) {DO;}

#define LOG(STR) if (opt->debug_p)  \
    printf("%sLOG%s: %s%s%s\n",     \
	   GREEN, NO_COLOR, RED, STR, NO_COLOR);

#define STR_EQ(STR1, STR2) (!strcmp(STR2, STR1))
#define EXP(DO, X)  if (X) {DO;}
...
void putc_int(int num, char *color){
  printf("%s%d%s", color, num, NO_COLOR);
}
...
void print_cont(char *prefix, char* container, char *arrow,
  ...
  printf("%s%s%s %s%s%s[%s%d/%d%s] %s%s%s %s%d%s\n",
	 GREEN,prefix,NO_COLOR,BLUE, container,NO_COLOR, RED, index, list_size,
	 NO_COLOR, BLUE,NO_COLOR,arrow, RED, new_elem.i, NO_COLOR);
}
...
void list_active_tests(arg_opt_t *opt){
    ...
    if (test_active(i, opt)) printf("Test Number: %d\nStatus: Active\n", i);
}
...
void print_help(){
  ...
  putc_str("-d --debug (-v)           (Enable debug/verbose output)", BLUE, true);
  putc_str("-t --run-tests            (Run tests)", BLUE, true);
  putc_str("-D --deep-debug           (More detailed debugging)", BLUE, true);	
  putc_str("-t --enable-test          (Enable part of a test or a test)", BLUE, true);
  putc_str("-s --set list_length <length>", BLUE, true);

  printf("       (Length of linked list <length> < 100)\n");
  
  putc_str("-e --enable-test  (Run separate tests (subtests) that checks diffrent things.", BLUE, true);
  ...
  printf("       (All integers between <lower> and <upper> will be used in the linked list)\n");

  putc_str("-s --set list_length <length>  (Set the length of the list)", BLUE, true);

  printf("       (This changes how long the used list is, but it must be lower than\n");
  printf("        the allocated list size (%d), and the values of the list defaults to\n", LIST_LEN);
...
void arg_parse(int argc, char **argv, arg_opt_t *opt){
  ...
  if(argc == 1){
	...
	// STX: --set <option> <value>
      }else if (STR_EQ(argv[i], "--use-test") ||
		STR_EQ(argv[i], "-e")){

	subtest_enable(atoi(argv[i + 1]), opt);       
	i++;
	
      }else if (STR_EQ(argv[i], "--set") ||
		STR_EQ(argv[i], "-s")){
	...
	if (STR_EQ(argv[i+1], "length")){
	  ...
	  if (-from + to > LIST_LEN) {
	    printf("%sERROR: Span can be a max of %d%s", RED,
		   LIST_LEN, NO_COLOR);
	    destroy_arg_opt(opt);
	    exit(-1);
	  }
	...
	// PARSE: Enable experimental features (mostly a lot of logs,..)
      }else if (STR_EQ(argv[i], "--deep-debug") ||
		STR_EQ(argv[i], "-D")){ 
	LOG("Experimental features enabled\n");
	...
	// PARSE: Enable running of test on start.
      }else if ((STR_EQ(argv[i], "--run-tests") ||
		 STR_EQ(argv[i], "-t"))){
	LOG("Running Tests...\n");
